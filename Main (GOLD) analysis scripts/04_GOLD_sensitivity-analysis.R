...
#
# This script performs sensitivity analyses for overall and subgroup prevalence of GOLD-PRISm, GOLD-RSP and 
# combined GOLD-PRISm and GOLD-RSP
#
# Studies were evaluated using JBI checklist for prevalence studies, Studies were included in the sensitivity
# analysis if they had a score >= 7 (high-quality studies)
#
...
summary(overall)
summary(overall$qa_score)

# Create GOLD dataset
gold_sens <- filter(overall, diag_defn == "gold" & qa_score >=7) # only take GOLD studies with QA score >=7
...
amro_gold_sens_comb <- filter(gold_sens, geo_location == "AMR")

amro_gold_sens_prism <- filter(gold_sens, diag == "prism" & geo_location == "AMR"& overall.prev == "y")

amro_gold_sens_rsp <- filter(gold_sens, diag == "rsp" & geo_location == "AMR"& overall.prev == "y")

# European region (EUR) - combined, prism and rsp groups
euro_gold_sens_comb <- filter(gold_sens, geo_location == "EUR" & id != 439)

euro_gold_sens_prism <- filter(gold_sens, diag == "prism" & geo_location == "EUR" & overall.prev == "y")

euro_gold_sens_rsp <- filter(gold_sens, diag == "rsp" & geo_location == "EUR" & overall.prev == "y")

# Western Pacific region (WPR) - combined, prism and rsp groups
wpro_gold_sens_comb <- filter(gold_sens, geo_location == "WPR")

wpro_gold_sens_prism <- filter(gold_sens, diag == "prism" & geo_location == "WPR" & overall.prev == "y")

wpro_gold_sens_rsp <- filter(gold_sens, diag == "rsp" & geo_location == "WPR" & overall.prev == "y")

# African region (AFR)) - combined, prism and rsp groups
afro_gold_sens_comb <- filter(gold_sens, geo_location == "AFR")

afro_gold_sens_prism <- filter(gold_sens, diag == "prism" & geo_location == "AFR" & overall.prev == "y")

afro_gold_sens_rsp <- filter(gold_sens, diag == "rsp" & geo_location == "AFR" & overall.prev == "y")

# Analysis code ##############################################################################################
#
# 1. Overall prevalence ####
#
# A. Combined PRISm and RSP
#
meta_comb_gold_sens <- comb_gold_sens %>%
  metaprop(
    ...
    prediction = TRUE,              # Compute prediction intervals
...
#
# Combined
meta_amro_gold_sens_comb <- amro_gold_sens_comb %>% 
  metaprop(
    event = case,         
    n = sample_size,          
    studlab = paste0(author),
...
forest(meta_amro_gold_sens_comb,
       rightlabs = c("Prevalence (%)", "95%-CI", "Weight (%)"), 
       leftlabs = c("Study","Cases","Total"))

# PRISm
meta_amro_gold_sens_prism <- amro_gold_sens_prism %>% 
  metaprop(
    event = case,         
    n = sample_size,          
    studlab = paste0(author),
...
forest(meta_amro_gold_sens_prism,
       rightlabs = c("Prevalence (%)", "95%-CI", "Weight (%)"), 
       leftlabs = c("Study","Cases","Total"))

# RSP
meta_amro_gold_sens_rsp <- amro_gold_sens_rsp %>% 
  metaprop(
    event = case,         
    n = sample_size,          
    studlab = paste0(author),
...
forest(meta_amro_gold_sens_rsp,
       rightlabs = c("Prevalence (%)", "95%-CI", "Weight (%)"), 
       leftlabs = c("Study","Cases","Total"))
...
#
# Combined
meta_euro_gold_sens_comb <- euro_gold_sens_comb %>% 
  metaprop(
    event = case,         
    n = sample_size,          
    studlab = paste0(author),
...
forest(meta_euro_gold_sens_comb,
       rightlabs = c("Prevalence (%)", "95%-CI", "Weight (%)"), 
       leftlabs = c("Study","Cases","Total"))

# PRISm
meta_euro_gold_sens_prism <- euro_gold_sens_prism %>% 
  metaprop(
    event = case,         
    n = sample_size,          
    studlab = paste0(author),
...
forest(meta_euro_gold_sens_prism,
       rightlabs = c("Prevalence (%)", "95%-CI", "Weight (%)"), 
       leftlabs = c("Study","Cases","Total"))

# RSP
meta_euro_gold_sens_rsp <- euro_gold_sens_rsp %>% 
  metaprop(
    event = case,         
    n = sample_size,          
    studlab = paste0(author),
...
forest(meta_euro_gold_sens_rsp,
       rightlabs = c("Prevalence (%)", "95%-CI", "Weight (%)"), 
       leftlabs = c("Study","Cases","Total"))
...
#
# Combined
meta_wpro_gold_sens_comb <- wpro_gold_sens_comb %>% 
  metaprop(
    event = case,         
    n = sample_size,          
    studlab = paste0(author),
...
forest(meta_wpro_gold_sens_comb,
       rightlabs = c("Prevalence (%)", "95%-CI", "Weight (%)"), 
       leftlabs = c("Study","Cases","Total"))

# PRISm
meta_wpro_gold_sens_prism <- wpro_gold_sens_prism %>% 
  metaprop(
    event = case,         
    n = sample_size,          
    studlab = paste0(author),
...
forest(meta_wpro_gold_sens_prism,
       rightlabs = c("Prevalence (%)", "95%-CI", "Weight (%)"), 
       leftlabs = c("Study","Cases","Total"))

# RSP
meta_wpro_gold_sens_rsp <- wpro_gold_sens_rsp %>% 
  metaprop(
    event = case,         
    n = sample_size,          
    studlab = paste0(author),
...
forest(meta_wpro_gold_sens_rsp,
       rightlabs = c("Prevalence (%)", "95%-CI", "Weight (%)"), 
       leftlabs = c("Study","Cases","Total"))
