...
# SCRIPT INTRODUCTION
# 
# This script performs the same function as gold_prevalence script, only with studies with 
# QA score >= 7.
# 
...
gold_sens_gender <- filter(overall, diag_defn == "gold"& qa_score >=7)         # have to separately do this for Cestelli et. al. 2025, which only reports prevalence in males

# sens_outlier = removing outliers in prevalence to check for outlier bias
gold_sens_outlier <- filter(overall, diag_defn == "gold") %>% # only including studies between 1st and 3rd quartile for overall prevalence values
  filter(!is.na(case), !is.na(sample_size), !is.na(geo_location))
...
prism_outlier_gold_sens <-filter(gold_sens_outlier, diag == "prism" & overall.prev == "y" & overall>=7.8 & overall<=14.1)

# rsp
rsp_gold_sens <-filter(gold_sens, diag == "rsp" & overall.prev == "y")
rsp_outlier_gold_sens <-filter(gold_sens_outlier, diag == "rsp" & overall.prev == "y" & overall>=7.8 & overall<=14.7)

# Combined prism and rsp 
comb_gold_sens <-filter(gold_sens, comb.prism.rsp.prev == "y")
comb_outlier_gold_sens <-filter(gold_sens_outlier, comb.prism.rsp.prev == "y" & overall>=9 & overall<=14.2)

#** 1.2.2 Subgroups - sex
#
# Males - combined, prism and rsp groups
male_gold_sens_comb <- filter(gold_sens_gender, gender.prev == "y") %>% 
  drop_na(male_total, male_cases)

male_gold_sens_comb <- male_gold_sens_comb[-25,]             # Remove rsp data from Cestelli et. al. 2025 (row 25) because PRISm is already taken from this study

male_gold_sens_prism <- filter(gold_sens_gender, diag == "prism" & gender.prev == "y") %>% 
  drop_na(male_total, male_cases)

male_gold_sens_rsp <- filter(gold_sens_gender, diag == "rsp" & gender.prev == "y") %>% 
  drop_na(male_total, male_cases)
...
# OUTLIER BIAS
# prism_outlier
prism_outlier_es <- escalc(
  measure = "PLO",
  xi = case,
  ni = sample_size,
  data = prism_outlier_gold_sens)

V_matrix_prism_outlier <- diag(prism_outlier_es$vi)

# rsp_outlier
rsp_outlier_es <- escalc(
  measure = "PLO",
  xi = case,
  ni = sample_size,
  data = rsp_outlier_gold_sens)

V_matrix_rsp_outlier <- diag(rsp_outlier_es$vi)

# comb_outlier
comb_outlier_es <- escalc(
  measure = "PLO",
  xi = case,
  ni = sample_size,
  data = comb_outlier_gold_sens)

V_matrix_comb_outlier <- diag(comb_outlier_es$vi)
}
...
# OUTLIER BIAS
prism_outlier_region_gold_sens_mlma_model <- rma.mv(
  yi = yi,
  V = V_matrix_prism_outlier,
  random = ~ 1 | geo_location/id,
  data = prism_outlier_es,
  method = "REML"
)

# Generate prediction intervals
pred_prism_outlier_region <- predict(
  prism_outlier_region_gold_sens_mlma_model,
  pi.type = "normal"
)

# Back-transform logit estimates to normal prevalence
pred_prev_prism_outlier_region <- data.frame(
  Predicted_Prevalence = plogis(pred_prism_outlier_region$pred) * 100,
  CI_Lower = plogis(pred_prism_outlier_region$ci.lb) * 100,
  CI_Upper = plogis(pred_prism_outlier_region$ci.ub) * 100,
  PI_Lower = plogis(pred_prism_outlier_region$pi.lb) * 100,
  PI_Upper = plogis(pred_prism_outlier_region$pi.ub) * 100
)
...
# OUTLIER BIAS
rsp_outlier_region_gold_sens_mlma_model <- rma.mv(
  yi = yi,
  V = V_matrix_rsp_outlier,
  random = ~ 1 | geo_location/id,
  data = rsp_outlier_es,
  method = "REML"
)
...
# Generate prediction intervals
pred_rsp_outlier_region <- predict(
  rsp_outlier_region_gold_sens_mlma_model,
  pi.type = "normal"
)

# Back-transform logit estimates to normal prevalence
pred_prev_rsp_outlier_region <- data.frame(
  Predicted_Prevalence = plogis(pred_rsp_outlier_region$pred) * 100,
  CI_Lower = plogis(pred_rsp_outlier_region$ci.lb) * 100,
  CI_Upper = plogis(pred_rsp_outlier_region$ci.ub) * 100,
  PI_Lower = plogis(pred_rsp_outlier_region$pi.lb) * 100,
  PI_Upper = plogis(pred_rsp_outlier_region$pi.ub) * 100
)
...
# OUTLIER BIAS
comb_outlier_region_gold_sens_mlma_model <- rma.mv(
  yi = yi,
  V = V_matrix_comb_outlier,
  random = ~ 1 | geo_location/id,
  data = comb_outlier_es,
  method = "REML"
)

# Generate prediction intervals
pred_comb_outlier_region <- predict(
  comb_outlier_region_gold_sens_mlma_model,
  pi.type = "normal"
)

# Back-transform logit estimates to normal prevalence
pred_prev_comb_outlier_region <- data.frame(
  Predicted_Prevalence = plogis(pred_comb_outlier_region$pred) * 100,
  CI_Lower = plogis(pred_comb_outlier_region$ci.lb) * 100,
  CI_Upper = plogis(pred_comb_outlier_region$ci.ub) * 100,
  PI_Lower = plogis(pred_comb_outlier_region$pi.lb) * 100,
  PI_Upper = plogis(pred_comb_outlier_region$pi.ub) * 100
)
...
# OUTLIER BIAS
prism_outlier_income_gold_sens_mlma_model <- rma.mv(
  yi = yi,
  V = V_matrix_prism_outlier,
  random = ~ 1 | income_class/id,
  data = prism_outlier_es,
...
# Generate prediction intervals
pred_prism_outlier_income <- predict(
  prism_outlier_income_gold_sens_mlma_model,
  pi.type = "normal"
)

# Back-transform logit estimates to normal prevalence
pred_prev_prism_outlier_income <- data.frame(
  Predicted_Prevalence = plogis(pred_prism_outlier_income$pred) * 100,
  CI_Lower = plogis(pred_prism_outlier_income$ci.lb) * 100,
  CI_Upper = plogis(pred_prism_outlier_income$ci.ub) * 100,
  PI_Lower = plogis(pred_prism_outlier_income$pi.lb) * 100,
  PI_Upper = plogis(pred_prism_outlier_income$pi.ub) * 100
)
...
# OUTLIER BIAS
rsp_outlier_income_gold_sens_mlma_model <- rma.mv(
  yi = yi,
  V = V_matrix_rsp_outlier,
  random = ~ 1 | income_class/id,
  data = rsp_outlier_es,
...
# Generate prediction intervals
pred_rsp_outlier_income <- predict(
  rsp_outlier_income_gold_sens_mlma_model,
  pi.type = "normal"
)

# Back-transform logit estimates to normal prevalence
pred_prev_rsp_outlier_income <- data.frame(
  Predicted_Prevalence = plogis(pred_rsp_outlier_income$pred) * 100,
  CI_Lower = plogis(pred_rsp_outlier_income$ci.lb) * 100,
  CI_Upper = plogis(pred_rsp_outlier_income$ci.ub) * 100,
  PI_Lower = plogis(pred_rsp_outlier_income$pi.lb) * 100,
  PI_Upper = plogis(pred_rsp_outlier_income$pi.ub) * 100
)
...
# OUTLIER BIAS
comb_outlier_income_gold_sens_mlma_model <- rma.mv(
  yi = yi,
  V = V_matrix_comb_outlier,
  random = ~ 1 | income_class/id,
  data = comb_outlier_es,
...
# Generate prediction intervals
pred_comb_outlier_income <- predict(
  comb_outlier_income_gold_sens_mlma_model,
  pi.type = "normal"
)

# Back-transform logit estimates to normal prevalence
pred_prev_comb_outlier_income <- data.frame(
  Predicted_Prevalence = plogis(pred_comb_outlier_income$pred) * 100,
  CI_Lower = plogis(pred_comb_outlier_income$ci.lb) * 100,
  CI_Upper = plogis(pred_comb_outlier_income$ci.ub) * 100,
  PI_Lower = plogis(pred_comb_outlier_income$pi.lb) * 100,
  PI_Upper = plogis(pred_comb_outlier_income$pi.ub) * 100
)
...
# OUTLIER BIAS
meta_prism_outlier_gold_sens <- prism_outlier_gold_sens %>%
  metaprop(
    event = case,      
    n = sample_size,         
...
# OUTLIER BIAS
meta_rsp_outlier_gold_sens <- rsp_outlier_gold_sens %>%
  metaprop(
    event = case,            
    n = sample_size,          
    studlab = paste0(author), 
...
# OUTLIER BIAS
meta_comb_outlier_gold_sens <- comb_outlier_gold_sens %>%
  metaprop(
    event = case,
    n = sample_size, 
    studlab = paste0(author),
...
# OUTLIER BIAS
# prism_outlier region
summary(prism_outlier_region_gold_sens_mlma_model)
# Extract total and level-specific I2 values
i2_prism_outlier_region_gold_sens_mlma_model<-var.comp(prism_outlier_region_gold_sens_mlma_model)
summary(i2_prism_outlier_region_gold_sens_mlma_model)

# rsp_outlier region
summary(rsp_outlier_region_gold_sens_mlma_model)
i2_rsp_outlier_region_gold_sens_mlma_model<-var.comp(rsp_outlier_region_gold_sens_mlma_model)
summary(i2_rsp_outlier_region_gold_sens_mlma_model)

# comb_outlier region
summary(comb_outlier_region_gold_sens_mlma_model)
i2_comb_outlier_region_gold_sens_mlma_model<-var.comp(comb_outlier_region_gold_sens_mlma_model)
summary(i2_comb_outlier_region_gold_sens_mlma_model)

# prism_outlier income
summary(prism_outlier_income_gold_sens_mlma_model)
i2_prism_outlier_income_gold_sens_mlma_model<-var.comp(prism_outlier_income_gold_sens_mlma_model)
summary(i2_prism_outlier_income_gold_sens_mlma_model)

# rsp_outlier income
summary(rsp_outlier_income_gold_sens_mlma_model)
i2_rsp_outlier_income_gold_sens_mlma_model<-var.comp(rsp_outlier_income_gold_sens_mlma_model)
summary(i2_rsp_outlier_income_gold_sens_mlma_model)

# comb_outlier income
summary(comb_outlier_income_gold_sens_mlma_model)
i2_comb_outlier_income_gold_sens_mlma_model<-var.comp(comb_outlier_income_gold_sens_mlma_model)
summary(i2_comb_outlier_income_gold_sens_mlma_model)
...
{
overall_prevalence_table <- bind_rows(
  data.frame(Type = "prism_outlier region", pred_prev_prism_outlier_region),
  data.frame(Type = "rsp_outlier region", pred_prev_rsp_outlier_region),
  data.frame(Type = "comb_outlier region", pred_prev_comb_outlier_region),
  data.frame(Type = "prism_outlier income", pred_prev_prism_outlier_income),
  data.frame(Type = "rsp_outlier income", pred_prev_rsp_outlier_income),
  data.frame(Type = "comb_outlier income", pred_prev_comb_outlier_income)
)
