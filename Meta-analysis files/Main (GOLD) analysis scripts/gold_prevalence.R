...
# SCRIPT INTRODUCTION
# 
# This script calculates and reports overall and subgroup prevalences of GOLD-PRISm, GOLD-RSP and 
# combined GOLD-PRISm and GOLD-RSP.
# 
# In this major revision, multi-level meta-analysis models have been used to estimate prevalence of
# GOLD-PRISm, GOLD-RSP and combined GOLD-PRISm and RSP.
# 
# Two sets of MLMA models have been created with WHO geographical regions and World Bank income 
# levels as clustering variables. Overall prevalence estimates from WHO region MLMA models are 
# reported in main-text and prevalence estimates from income-level MLMA models are reported in 
# supplementary files. Pooled prevalence estimates from standard random-effects models will also be 
# reported in supplementary files for comparison.
# 
# Subgroup prevalences for WHO regions and World Bank income-levels are derived from their 
# respective MLMA models. For gender and smoking habit subgroups, prevalence was calculated via 
# standard random-effect models.This is due to the hierarchical structure of data - our studies can 
# be nested into different regions/income groups due to their global distribution. It is not 
# possible to do the same with genders and smoking habits.
# 
# 
...
#
#* 1.1 Data import and filtering by gold definition ------------------------------------------------
...
# Create GOLD dataset - for main analysis
gold <- filter(overall, diag_defn == "gold") %>% 
  filter(!is.na(case), !is.na(sample_size), !is.na(geo_location))

gold_gender <- filter(overall, diag_defn == "gold")         # have to separately do this for Cestelli et. al. 2025, which only reports prevalence in males
...
male_gold_comb <- filter(gold_gender, gender.prev == "y") %>% 
  drop_na(male_total, male_cases)

male_gold_comb <- male_gold_comb[-32,]             # Remove rsp data from Cestelli et. al. 2025 (row 25) because PRISm is already taken from this study

male_gold_prism <- filter(gold_gender, diag == "prism" & gender.prev == "y") %>% 
  drop_na(male_total, male_cases)

male_gold_rsp <- filter(gold_gender, diag == "rsp" & gender.prev == "y") %>% 
  drop_na(male_total, male_cases)
...
# 
#*** 2.1.1.1 prism ####
prism_region_gold_mlma_model <- rma.mv(
...
#*** 2.1.1.2 rsp ####
rsp_region_gold_mlma_model <- rma.mv(
  yi = yi,
  V = V_matrix_rsp,
...
#*** 2.1.1.3 combined ####
comb_region_gold_mlma_model <- rma.mv(
  yi = yi,
  V = V_matrix_comb,
  random = ~ 1 | geo_location/id,
  data = comb_es,
...
geo_location_prism_effects <- ranef(prism_region_gold_mlma_model)$geo_location
fixed_intercept_prism_region <- prism_region_gold_mlma_model$b[1, 1]
...
# For PI
residual_variance <- prism_region_gold_mlma_model$sigma2[1]
...
#
# Extract random effects at geo_location level and fixed intercept
geo_location_rsp_effects <- ranef(rsp_region_gold_mlma_model)$geo_location
fixed_intercept_rsp_region <- rsp_region_gold_mlma_model$b[1, 1]
...
# For PI
residual_variance <- rsp_region_gold_mlma_model$sigma2[1]
...
#
# Extract random effects at geo_location level and fixed intercept
geo_location_comb_effects <- ranef(comb_region_gold_mlma_model)$geo_location
fixed_intercept_comb_region <- comb_region_gold_mlma_model$b[1, 1]
...
# For PI
residual_variance <- comb_region_gold_mlma_model$sigma2[1]
...
#
#*** 2.1.2.1 prism ====
prism_income_gold_mlma_model <- rma.mv(
  yi = yi,
  V = V_matrix_prism,
...
#*** 2.1.2.2 rsp ====
rsp_income_gold_mlma_model <- rma.mv(
  yi = yi,
  V = V_matrix_rsp,
  random = ~ 1 | income_class/id,
  data = rsp_es,
...
#*** 2.1.2.3 combined ====
comb_income_gold_mlma_model <- rma.mv(
  yi = yi,
  V = V_matrix_comb,
  random = ~ 1 | income_class/id,
  data = comb_es,
...
income_class_prism_effects <- ranef(prism_income_gold_mlma_model)$income_class
fixed_intercept_prism_income <- prism_income_gold_mlma_model$b[1, 1]
...
# For PI
residual_variance <- prism_income_gold_mlma_model$sigma2[1]
...
#
# Extract random effects at income_class level and fixed intercept
income_class_rsp_effects <- ranef(rsp_income_gold_mlma_model)$income_class
fixed_intercept_rsp_income <- rsp_income_gold_mlma_model$b[1, 1]
...
# For PI
residual_variance <- rsp_income_gold_mlma_model$sigma2[1]
...
#
# Extract random effects at income_class level and fixed intercept
income_class_comb_effects <- ranef(comb_income_gold_mlma_model)$income_class
fixed_intercept_comb_income <- comb_income_gold_mlma_model$b[1, 1]
...
# For PI
residual_variance <- comb_income_gold_mlma_model$sigma2[1]
...
#
# prism region
summary(prism_region_gold_mlma_model)
# Extract total and level-specific I2 values
i2_prism_region_gold_mlma_model<-var.comp(prism_region_gold_mlma_model)
summary(i2_prism_region_gold_mlma_model)

# rsp region
summary(rsp_region_gold_mlma_model)
i2_rsp_region_gold_mlma_model<-var.comp(rsp_region_gold_mlma_model)
summary(i2_rsp_region_gold_mlma_model)

# comb region
summary(comb_region_gold_mlma_model)
i2_comb_region_gold_mlma_model<-var.comp(comb_region_gold_mlma_model)
summary(i2_comb_region_gold_mlma_model)

# prism income
summary(prism_income_gold_mlma_model)
i2_prism_income_gold_mlma_model<-var.comp(prism_income_gold_mlma_model)
summary(i2_prism_income_gold_mlma_model)

# rsp income
summary(rsp_income_gold_mlma_model)
i2_rsp_income_gold_mlma_model<-var.comp(rsp_income_gold_mlma_model)
summary(i2_rsp_income_gold_mlma_model)

# comb income
summary(comb_income_gold_mlma_model)
i2_comb_income_gold_mlma_model<-var.comp(comb_income_gold_mlma_model)
summary(i2_comb_income_gold_mlma_model)

# overall standard random-effects ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# prism
summary(meta_prism_gold)
...
# Gender - females
# females - prism
summary(meta_female_gold_prism)
...
# Smoking habit ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Non-smokers
summary(meta_non_smoker_gold_prism)

forest(meta_non_smoker_gold_prism,
        rightlabs = c("Prevalence (%)", "95%-CI", "Weight (%)"),
        leftlabs = c("Study","Cases","Total"))

summary(meta_non_smoker_gold_rsp)

forest(meta_non_smoker_gold_rsp,
        rightlabs = c("Prevalence (%)", "95%-CI", "Weight (%)"),
        leftlabs = c("Study","Cases","Total"))

summary(meta_non_smoker_gold_comb)
...
# ex-smokers
summary(meta_ex_smoker_gold_prism)

forest(meta_ex_smoker_gold_prism,
       rightlabs = c("Prevalence (%)", "95%-CI", "Weight (%)"),
       leftlabs = c("Study","Cases","Total"))

summary(meta_ex_smoker_gold_rsp)

forest(meta_ex_smoker_gold_rsp,
       rightlabs = c("Prevalence (%)", "95%-CI", "Weight (%)"),
       leftlabs = c("Study","Cases","Total"))
summary(meta_ex_smoker_gold_comb)
...
# current smokers
summary(meta_cur_smoker_gold_prism)

forest(meta_cur_smoker_gold_prism,
       rightlabs = c("Prevalence (%)", "95%-CI", "Weight (%)"),
       leftlabs = c("Study","Cases","Total"))

summary(meta_cur_smoker_gold_rsp)

forest(meta_cur_smoker_gold_rsp,
       rightlabs = c("Prevalence (%)", "95%-CI", "Weight (%)"),
       leftlabs = c("Study","Cases","Total"))

summary(meta_cur_smoker_gold_comb)
...
# Apply to each model
res01 <- extract_prevalence_results(meta_prism_gold, prism_gold, "prism gold")
res02 <- extract_prevalence_results(meta_rsp_gold, rsp_gold, "rsp gold")
res03 <- extract_prevalence_results(meta_comb_gold, comb_gold, "combined gold")
res04 <- extract_prevalence_results(meta_male_gold_prism, male_gold_prism, "male prism")  
res05 <- extract_prevalence_results(meta_male_gold_rsp, male_gold_rsp, "male rsp")  
res06 <- extract_prevalence_results(meta_male_gold_comb, male_gold_comb, "male comb")  
res07 <- extract_prevalence_results(meta_female_gold_prism, female_gold_prism, "female prism")  
res08 <- extract_prevalence_results(meta_female_gold_rsp, female_gold_rsp, "female rsp")
res09 <- extract_prevalence_results(meta_female_gold_comb, female_gold_comb, "female comb")
res10 <- extract_prevalence_results(meta_non_smoker_gold_prism, non_smoker_gold_prism, "non-smoker prism")
res11 <- extract_prevalence_results(meta_non_smoker_gold_rsp, non_smoker_gold_rsp, "non-smoker rsp")
res12 <- extract_prevalence_results(meta_non_smoker_gold_comb, non_smoker_gold_comb, "non-smoker comb")
res13 <- extract_prevalence_results(meta_ex_smoker_gold_prism, ex_smoker_gold_prism, "ex-smoker prism")
res14 <- extract_prevalence_results(meta_ex_smoker_gold_rsp, ex_smoker_gold_rsp, "ex-smoker rsp")
res15 <- extract_prevalence_results(meta_ex_smoker_gold_comb, ex_smoker_gold_comb, "ex-smoker comb")
res16 <- extract_prevalence_results(meta_cur_smoker_gold_prism, cur_smoker_gold_prism, "current smoker prism")
res17 <- extract_prevalence_results(meta_cur_smoker_gold_rsp, cur_smoker_gold_rsp, "current smoker rsp")
res18 <- extract_prevalence_results(meta_cur_smoker_gold_comb, cur_smoker_gold_comb, "current smoker comb")
...
mu_hat_prism_region <- as.numeric(coef(prism_region_gold_mlma_model))
...
{
# Get the pooled effect estimate from your MLMA model
mu_hat_rsp_region <- as.numeric(coef(rsp_region_gold_mlma_model))
...
{
# Get the pooled effect estimate from your MLMA model
mu_hat_comb_region <- as.numeric(coef(comb_region_gold_mlma_model))
